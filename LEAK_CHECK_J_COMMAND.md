# 漏液チェック機能の仕様変更（jコマンド実装）

## 概要

漏液チェック機能をリファクタリングしました。従来のポーリング方式（回転速度取得コマンド（Xコマンド）のおまけ処理）から、専用の状態確認コマンド（jコマンド）を使用する方式に変更しました。

**注**: Zコマンドは緊急停止（緊急停止：すべてのポンプ停止）として予約されているため、状態確認には小文字の'j'コマンドを使用します。
...
```
【新方式】jコマンド（状態確認）
Python側 ---(jコマンド)---> Arduino側
          STX/0/j/000000/CS/ETX
...
```
フレーム構成：STX / ポンプNo / j / 値 / CS / ETX
例:           0x02 / '0' / 'j' / "000000" / CS / 0x03
...
```
正常時:
フレーム構成：STX / 'J' / 状態 / CS / ETX
例:           0x02 / 'J' / "000000" / CS / 0x03

漏液検出時:
フレーム構成：STX / 'J' / 状態 / CS / ETX
例:           0x02 / 'J' / "000001" / CS / 0x03
...
```cpp
else if (action == 'j') {  // 状態確認コマンド
    char response[11];
    response[0] = 0x02;  // STX
    response[1] = 'J';   // 応答タイプ（大文字）
...
**処理フロー:**
1. シリアルバッファをクリア
2. jコマンドを送信 (`send_serial_command(1, "j", "000000")`)
3. 応答待機（最大1秒）
4. 応答フレーム解析（応答フレームのポンプNo位置は'J'）
5. ステータス値の最下位ビット（bit0）で漏液判定
6. グローバル変数 `leak_detected` を更新

### フロントエンド (`pump_control.html`)

**変更点:**
- `checkLeakStatus()` で `/api/check_leak_status` を呼び出し
- 定期実行（5秒間隔）
...
✅ **シンプル**: jコマンド（小文字）で識別性向上
✅ **応答明確**: 応答ポンプNo位置を'J'（大文字）に
✅ **シリアルバッファの分離**: 各コマンドが独立して動作
✅ **信頼性**: チェックサム検証でデータ整合性確保
✅ **拡張性**: ビット1-5で他の状態も追加可能
...
```
[ACM0(COM18)] jコマンド送信完了。応答待機中...
[ACM0(COM18)] jコマンド応答受信: 02 4A 30 30 30 30 30 31 E8 03 (10 bytes)
状態確認完了: 漏液検出 - 漏液: True
```
